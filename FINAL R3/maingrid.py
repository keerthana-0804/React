...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, size=5):
        ...
        self.reward_map = { # The reward                                                                                        
            ...
            "furnace":-10  #Reward for reaching furnace
        }
        self.total_reward=0                                                                                                    
        ...
        self.collected_images = {
            "wood": pygame.image.load("img\cw.jpg"),
            "iron": pygame.image.load("img\ci.jpg"),
            "grass": pygame.image.load("img\cgrass.jpg"),
            "toolshed": pygame.image.load("img\ct.jpg"),
            "gold": pygame.image.load("img\cg.jpg"),
            "furnace": pygame.image.load("img\cw.jpg")
        }
        self.item_images = {
            "wood": pygame.image.load("img\minewood.png"),
            "iron": pygame.image.load("img\mineiron.png"),
            "grass": pygame.image.load("img\minegrass.png"),
            "toolshed": pygame.image.load("img\mineshed.png"),
            "gold": pygame.image.load("img\gold.png"),
            "furnace": pygame.image.load("img\minefurnace.png")  # Load the furnace image
        }
    ...
    def _initialize_item_locations(self):
        ...
        for item_name in ["wood", "iron", "grass", "toolshed", "gold","furnace"]:
    ...
    def _get_obs(self):
        return {
            ...
            "furnace":self._furnace_location,
            }
    ...
    def step(self, action,uinput,start_time):
        ...
        if np.array_equal(new_agent_location, self._wood_location):
            ...
            item_type = "gold"
        elif np.array_equal(new_agent_location, self._furnace_location):
            item_type = "furnace"
    ...
    def _render_frame(self):
        ...
        for item, location in self._get_obs().items():
            if location is not None and item != "agent" and item != "target":
                if item in self.collected_items:
                    # If the item is collected, display the collected image
                    item_image = pygame.transform.scale(self.collected_images[item], (pix_square_size, pix_square_size))
                else:
                    # If the item is not collected, display the regular item image
                    item_image = pygame.transform.scale(self.item_images[item], (pix_square_size, pix_square_size))
        ...
        # Draw agent
        agent_image = pygame.image.load("img\mineplayer.png")  # Replace with your default agent image file
        agent_image = pygame.transform.scale(agent_image, (pix_square_size, pix_square_size))
        
        # Check if the agent is one grid away from any item and change the agent's picture
        for item, location in self._get_obs().items():
            if location is not None and item != "agent" and item != "target":
                distance = abs(location[0] - self._agent_location[0]) + abs(location[1] - self._agent_location[1])
                if distance == 1:
                    # If the agent is one grid away from any item, change the agent's picture
                    agent_image = pygame.image.load("img\mineplayer.png")  # Replace with your new agent image file
                    agent_image = pygame.transform.scale(agent_image, (pix_square_size, pix_square_size))
                    break
    ...
    def convert_to_graph(self, uloclist):
        ...
        for item_location in [self._wood_location, self._iron_location, self._grass_location, self._toolshed_location, self._gold_location,self._furnace_location]:
    ...
    def display_graph(self):
        ...
        for node in self.graph.nodes():
            ...
            elif tuple(node) == tuple(self._furnace_location):
                node_colors.append('orange')  # Color for gold
            else:
