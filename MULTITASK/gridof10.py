...
class GridWorldEnv(gym.Env):
    ...
    def _initialize_item_locations(self):
        # Initialize empty lists to store assigned locations for each item
        self.assigned_locations = {'wood': [], 'iron': [], 'grass': [], 'toolshed': [], 'gold': []}

        # Place 2 instances of each item
        for item_name in self.assigned_locations.keys():
            for _ in range(2):
                ...
                while location in self.assigned_locations[item_name]:
                    x = random.randint(0, self.size - 1)
                    y = random.randint(0, self.size - 1)
                    location = (x, y)

                # Update the assigned locations list
                self.assigned_locations[item_name].append(location)

                # Set the location of the item
                setattr(self, f"_{item_name}_location", location)

        print("ASSIGNED LOCATIONS",self.assigned_locations)
            # Set the location of each item
        for item_name, locations in self.assigned_locations.items():
            for i, location in enumerate(locations):
                setattr(self, f"_{item_name}_location_{i}", location)
    ...
    def _render_frame(self):
        ...
        for item_name, locations in self.assigned_locations.items():
            for i, location in enumerate(locations):
                if location is not None and item_name != "agent" and item_name != "target":
                    # Load image and scale it to fit grid square
                    item_image = pygame.transform.scale(item_images[item_name], (pix_square_size, pix_square_size))
