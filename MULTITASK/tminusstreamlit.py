...
def display_single_task_options():
    ...
    if st.button(" 💻 Show Environment"):
        ...
        script_path = "C:\\Users\\Keerthana\\Documents\\FROZENLAKE ENV\\execbestpath.py"
...
def display_multi_task_options():
    ...
    if st.button(" 💻 Show Environment"):
        ...
        process = subprocess.run(["python", "tminuspt2.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def display_existing_approaches():
    ...
    if st.button("Run Env"):
        st.write("Script is running")
        process = subprocess.run(["python", "thousandandfifth.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
